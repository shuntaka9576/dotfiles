...
- **`git add` を絶対に実行しない** - ステージ済みの変更のみを対象とする
- **未ステージのファイルには触れない** - 完全にそのままにする
- ステージされた変更がない場合は、ユーザーに通知して何もしない
...
- ステージされた変更がない場合は、処理を中止してユーザーに通知: 「ステージされた変更がありません。先に `git add` で変更をステージしてください。」
